...
package org.apache.amber.operator.visualization.filledAreaPlot
  ...
  override def generatePythonCode(): String = {
    val finalCode =
      s"""
         ...
         |            yield self.render_error_with_json("X attribute is not shared across all line groups")
         |        elif error == "missing attributes":
         |            yield self.render_error_with_json("X or Y attribute does not exist")
         |""".stripMargin
    finalCode
  }
